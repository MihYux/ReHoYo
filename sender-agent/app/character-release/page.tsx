...
function ReleaseStep({ task, regionName, stopped, rollout, setRollout, publish, busy }: {
  ...
  return <><div className={styles.releaseSummary}><div><span>当前区域方案</span><h2>{task.title}</h2><p>{task.theme}</p></div><dl><div><dt>区域指南</dt><dd>{regionName}</dd></div><div><dt>来源</dt><dd>{task.sourceDocument?.name || "控制台创建"}</dd></div><div><dt>状态</dt><dd>{task.status === "ready" ? "可发布" : "草稿"}</dd></div></dl></div><section className={styles.card}><div className={styles.cardHead}><div><span>全局灰度控制</span><h3>设置所有区域桌宠的问候比例</h3></div><strong className={styles.rolloutValue}>{rollout}%</strong></div><div className={styles.presets}>{[1, 5, 10, 25, 50, 100].map((value) => <button key={value} className={rollout === value ? styles.selected : ""} onClick={() => setRollout(value)}>{value}%</button>)}</div><input className={styles.range} type="range" min="1" max="100" value={rollout} onChange={(event) => setRollout(Number(event.target.value))} /><p className={styles.muted}>每次点击都会更新当前所选区域的在线指南，并另外发布一条全局问候命令。所有区域桌宠都会读取该命令：100% 全部命中，50% 稳定命中约一半；每只桌宠继续遵循自身已同步的区域指南和用户设置。</p></section><div className={styles.publishActions}><button className="button button-primary" disabled={stopped || task.status !== "ready" || Boolean(busy)} onClick={() => publish(false)}><PaperPlaneTilt weight="fill" />按 {rollout}% 向全部区域发送</button><button className={styles.example} disabled={stopped || task.status !== "ready" || Boolean(busy)} onClick={() => publish(true)}><Sparkle weight="fill" />全部区域 · 100%</button></div></>;
}
...
function OptimizationStep({ task, regionName, release }: { task: CharacterReleaseTask | null; regionName: string; release: CharacterPlanRelease | null }) {
  ...
  if (!task || !release || !report) return <Empty title="发布后生成效果视图" text="完成一次灰度或全量问候发布后，这里会显示稳定、可复核的模拟聚合指标。" />;
