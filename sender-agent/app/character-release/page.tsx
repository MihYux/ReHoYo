...
export default function CharacterReleasePage() {
  ...
  const publish = (exampleMode: boolean) => {
    ...
    return mutate(exampleMode ? "example" : "publish", () => call("/api/character-release/publish", {
      ...
      : `自然问候已按 ${rollout}% 灰度上传到 Cloudflare；命中且允许主动联系的桌宠将在约 1 分钟内发送。`);
  };
...
function ReleaseStep({ task, regionName, stopped, rollout, setRollout, publish, busy }: {
  ...
  return <><div className={styles.releaseSummary}><div><span>当前区域方案</span><h2>{task.title}</h2><p>{task.theme}</p></div><dl><div><dt>区域</dt><dd>{regionName}</dd></div><div><dt>来源</dt><dd>{task.sourceDocument?.name || "控制台创建"}</dd></div><div><dt>状态</dt><dd>{task.status === "ready" ? "可发布" : "草稿"}</dd></div></dl></div><section className={styles.card}><div className={styles.cardHead}><div><span>灰度控制</span><h3>设置本次自然问候比例</h3></div><strong className={styles.rolloutValue}>{rollout}%</strong></div><div className={styles.presets}>{[1, 5, 10, 25, 50, 100].map((value) => <button key={value} className={rollout === value ? styles.selected : ""} onClick={() => setRollout(value)}>{value}%</button>)}</div><input className={styles.range} type="range" min="1" max="100" value={rollout} onChange={(event) => setRollout(Number(event.target.value))} /><p className={styles.muted}>每次点击都会生成新的发布批次。桌宠按自身区域和稳定灰度决定是否发送符合在线指南的日常问候；100% 全量命中，50% 稳定命中约一半。用户暂停、勿扰、拒绝和主动联系设置始终有效。</p></section><div className={styles.publishActions}><button className="button button-primary" disabled={stopped || task.status !== "ready" || Boolean(busy)} onClick={() => publish(false)}><PaperPlaneTilt weight="fill" />按 {rollout}% 发布自然问候</button><button className={styles.example} disabled={stopped || task.status !== "ready" || Boolean(busy)} onClick={() => publish(true)}><Sparkle weight="fill" />全量问候 · 100%</button></div></>;
}
