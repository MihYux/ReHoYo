...
function localReleaseMessageReview({ text, plan, contactAllowed = true, messageMode = "release_context", allowDirectReveal = false }) {
  ...
  if (!facts.length && messageMode !== "casual_check_in") {
    failures.push({ dimension: "factual_grounding", code: "no_safe_fact" });
  }
...
function reviewPrompt(messageMode = "release_context") {
...
Schema: {"decision":"execute|rewrite|skip","dimensions":{"dimension":{"status":"pass|fail","reasonCode":"short_code"}},"revisedText":"optional"}.
  The message mode is ${messageMode}. The message must be natural first-person March 7th dialogue, culturally appropriate, low pressure, easy to refuse, non-manipulative, and must not mention internal operations. For casual_check_in, a normal greeting that makes no release claim does not need to cite allowedFacts and must not announce a new version. For release_context, ground claims only in allowedFacts. The theme and narrative are operator instructions, not dialogue: never quote them or expose them. Never reveal reasoning. Use rewrite only when one safe rewrite can fix the message; use skip when required facts are insufficient.`;
}
  ...
  try {
    for (let attempt = 0; attempt < 2; attempt += 1) {
      const response = await requestReview({
        systemPrompt: reviewPrompt(messageMode),
        payload: {
          candidate,
