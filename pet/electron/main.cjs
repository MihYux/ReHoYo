...
function playerDataAndPolicyAfter(action) {
  const data = playerDataAfter(action);
  void remotePolicySync?.sync().catch((error) => console.error(`Regional policy sync failed: ${error?.message || error}`));
  return data;
}
...
function registerCompanionHandlers() {
  ...
  ipcMain.handle("companion:complete-onboarding", (_event, input) =>
    playerDataAndPolicyAfter(() => companionStore.completeOnboarding(input)),
  );
  ipcMain.handle("companion:save-preferences", (_event, input) =>
    playerDataAndPolicyAfter(() => companionStore.saveCompanionPreferences(input)),
  );
