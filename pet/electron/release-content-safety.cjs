...
function metadataReason(value, field = "fact") {
...
function validateReleasePlanFields(plan = {}) {
  const errors = [];
  for (const field of ["title", "theme", "narrative", "timeWindow"]) {
    const reason = metadataReason(plan[field], field);
    if (reason && reason !== "empty") errors.push({ field, reason });
  }
  for (const [index, fact] of (Array.isArray(plan.facts) ? plan.facts : []).entries()) {
    for (const key of ["label", "value", "source"]) {
      const reason = metadataReason(fact?.[key], "fact");
...
module.exports = { metadataReason, validateReleasePlanFields };
