...
export function ModelSettingsPanel({
  ...
  const [policyStatus, setPolicyStatus] = useState<RemotePolicyStatus | null>(null);
  ...
  const policyApi = window.marchDesktop?.policy;
  ...
  useEffect(() => {
    policyApi?.getStatus().then(setPolicyStatus).catch(() => setPolicyStatus(null));
  }, [policyApi]);

  const syncPolicy = async () => {
    if (!policyApi) return;
    setPolicyBusy(true);
    const result = await policyApi.sync();
    if (result.status) setPolicyStatus(result.status);
    setNotice(result.ok
      ? { kind: "success", text: result.result.status === "updated" ? "区域策略已更新并应用" : "当前已是最新区域策略" }
  ...
  return (
    ...
    >
      ...
      <div className="settings-section-title">区域同行策略</div>
      <section className="default-scale-setting">
        <header>
          <CloudArrowDown weight="bold" />
          <span>
            <strong>{policyStatus?.regionCode || "--"} 区域策略</strong>
            <small>{policyStatus?.configured ? `版本 ${policyStatus.policyVersion}` : "云端暂未提供此区域策略"}</small>
          </span>
          <button type="button" className="settings-action-button" disabled={policyBusy || !policyApi} onClick={() => void syncPolicy()}>{policyBusy ? <SpinnerGap className="spin" /> : "立即同步"}</button>
        </header>
        <div className="provider-endpoint"><span className={`provider-dot ${policyStatus?.configured ? "is-ready" : ""}`} /><code>{policyStatus?.serviceUrl || "https://rehoyo.ccwu.cc"}</code></div>
      </section>
